...
- **Signing out now resets the app, told by the page instead of guessed from the chrome.** `native_identity_tag current_user&.id` in the layout says who is signed in, as an opaque token that never exposes the id (keyed by `secret_key_base`, so it also can't be compared across apps). When the value empties (sign out) or changes (an account or team switch — pass `[user.id, team.id]` to make the team part of the boundary), every tab is torn down so the previous session's screens are unreachable — including signing out from a page with no tab bar, which nothing caught before. Signing in never resets: a login reached deep in a checkout flow keeps its place, and the tabs rebuild only when there was a signed-out screen behind them. Pages that hide the tab bar now only hide the tab bar — the page, its back button, and the other tabs stay put. Render the tag on every page, signed out included: the signed-out page emitting an empty value is how sign-out is stated. Both modes, iOS and Android. Inertia apps share `native_identity_token(current_user&.id)` as a prop and render `NativeIdentity` from the `createInertiaApp` resolve wrapper — the wrapper rather than the main layout, because auth pages opt out of layouts, and a signed-out page missing the element makes sign-out invisible to the app. Apps that don't render the tag keep the old behavior exactly.

- **Links to another tab's path now switch tabs on Android in Normal Mode**, closing the last tab-routing platform gap: Turbo visits, Inertia visits, and plain anchor clicks all route, exactly as on iOS, and the `auto_route` options apply as documented. Back navigation matches iOS now too: pressing back skips form pages and never resurfaces a page from before a sign-in. This changes what the back button walks through in existing Android apps — it follows the app's navigation history rather than the WebView's raw page list — and arrives with the next build.

- **Advanced Mode on Android keeps the page when it hides the tab bar.** Opening a screen that declares `hide_native_tabs` no longer restarts every tab at its start URL; popping back restores the bar over the tab you came from. The iOS half of this shipped in 0.11.2.

- **Builds and screenshot captures now use the native code that matches your gem.** Cloud builds used to check out the newest native release no matter which gem the app runs, so a new native minor silently reached every next build. They now resolve the newest native release inside your gem's minor: patch fixes still arrive automatically, and a new minor arrives when you update the gem. Screenshot captures resolve the same way, so a capture previews what your next build actually ships.
...
- **`ruby_native deploy --if-needed` no longer crashes before an app's first successful build**, and it now compares against the latest build for the platform it is deploying, so `--android --if-needed` builds instead of skipping because an iOS build already matched.
...
4. **Recommended: adopt the identity tag.** Add `<%= native_identity_tag current_user&.id %>` to your layout, outside any signed-in check, and rebuild. This is what makes sign-out reset the app from any page and lets `hide_native_tabs` pages keep their back button. Skipping it is safe — without the tag the app keeps the old behavior. Inertia apps: see [the sign-out docs](https://rubynative.com/docs/authentication) for the resolve-wrapper pattern; the tag must not live only in a layout component that auth pages opt out of.
