...
- **Linked domains now work on Android.** Add an `android` section to `config/ruby_native.yml` with your package name and the SHA-256 fingerprint from Play Console → Setup → App integrity, and the gem serves `/.well-known/assetlinks.json`. Links to your site then open the app instead of the browser, including QR codes scanned from the camera, and saved passwords fill in the app. Needs an app built after this release. [Docs](https://rubynative.com/docs/linked-domains)
